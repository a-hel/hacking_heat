...
def build_mlp(input_var=None, channels=1, size=(28,28)):
    ...
    l_in = lasagne.layers.InputLayer(shape=(None, channels, size[0], size[1]),
                                     input_var=input_var)
...
def iterate_minibatches(inputs, targets, batchsize, shuffle=False):
    ...
    print(range(0, len(inputs) - batchsize + 1, batchsize))
    print(len(inputs))
...
def main(X_train, y_train, X_val, y_val, X_test, 
    ...
    if model == 'mlp':
        network = build_mlp(input_var, channels=channels, size=size)
