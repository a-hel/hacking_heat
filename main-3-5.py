...
Untested! Please report bugs as GitHub issues or so.
"""
...
def _get_img(path):
	...
	if path.startswith("http"):
		imgRequest = urllib.request.Request(path)#, headers=headers)
		try:
...
def build_network(f_train, f_val, f_test, img_size, greyscale=False, flatten=False,
	...
	pred_table = list(zip(test_urls, pred_names))
	return pred_table
